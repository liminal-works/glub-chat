...
//
// v1 is static colors + bold/italic/underline/strike + reset. Animated codes
// (&k obfuscated, &g rainbow) are deliberately deferred to a fast-follow, so
// their letters are NOT recognized here and read as literal text for now.
...
// non-color format codes we honor in v1. (k=obfuscated and g=rainbow are
// animated and intentionally omitted - they fall through as literal text.)
const FORMATS = new Set(["l", "m", "n", "o"]); // bold / strikethrough / underline / italic
const RESET = "r";

// a char is a recognized code iff it's a color, a format, or reset. Uppercase
// never matches, so acronyms ("Q&A", "AT&T") are safe even at a boundary.
function isCodeChar(ch) {
	return Object.prototype.hasOwnProperty.call(COLORS, ch) || FORMATS.has(ch) || ch === RESET;
}
...
// the Minecraft legacy rule: a color code also clears every format flag; a
// format code adds one; reset clears everything. Applied per code token.
function applyCode(state, c) {
	if (c === RESET) return { color: null, bold: false, italic: false, underline: false, strike: false };
	if (Object.prototype.hasOwnProperty.call(COLORS, c)) {
		return { color: COLORS[c], bold: false, italic: false, underline: false, strike: false };
	}
	const next = { ...state };
